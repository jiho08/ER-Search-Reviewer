...
2026-09-16에 [공개 사이트](https://lumia-er-reviewer.57872222k.workers.dev/)의 실제 ER 검색·시즌 전적·규칙 기반 리뷰를 검증했다. Worker 이름은 `lumia-er-reviewer`이며 `ER_API_KEY` Secret이 적용되어 있다. 상세 기록이 없는 플레이어의 검색 수정도 공개 배포하고 현재·과거 시즌 조회를 확인했다. 사용자 지정 도메인은 연결하지 않았고 `workers.dev` 주소를 사용한다. 기존 로컬 실행과 Codex 설정은 유지한다.
...
5. 생성된 Worker의 **Settings → Variables and Secrets**에서 `ER_API_KEY`를 **Secret**으로 등록하고 **Deploy**로 적용한다. 로컬 `.env`를 GitHub에 올리지 않는다.
...
공개 주소의 `/api/config`에서 `gameApiConfigured`를 확인한다. 이 값은 서버에 공백이 아닌 키가 전달됐는지만 보여 주며 키 값이나 인증 성공 여부는 공개하지 않는다. `false`이면 현재 Worker의 Secret 이름과 적용한 배포를 확인한다. `true`인데 특정 닉네임만 실패하면 키를 다시 등록하기 전에 실제 검색 오류를 구분해야 한다.

2026-09-16에는 키가 정상 적용된 상태에서 기존 플레이어의 최근 경기 응답이 404여서 검색이 실패했다. 현재 공개 버전은 경기 목록의 404만 빈 상세 기록으로 처리하고 시즌 목록과 API가 제공하는 누적 성적은 계속 조회한다. 이전 시즌을 선택해도 API가 그 UID에 대해 누적 기록을 제공하지 않으면 임의로 복원하지 않는다. 인증·접근 오류, 호출 제한, 다른 서버 오류는 빈 경기로 바꾸지 않는다. 공개 서버에서 상세 기록 없는 플레이어의 현재·과거 시즌 조회가 HTTP 200임을 확인했다.
...
무료 Workers에는 요청 수·CPU 처리 시간 제한이 있다. 실제 공개 주소의 단일 사용자 흐름은 확인했지만 다수 사용자 부하와 무료 범위의 충분성은 아직 검증하지 않았다. AI를 꺼도 서버 실행·저장 비용은 발생할 수 있다.

2026-09-16 공개 검증: 명시적 HTTP 검사 13개와 smoke 검사를 통과했다. 키가 등록되어 미설정 키 오류 항목은 건너뛰었다. 물방개 시즌 12의 7,186 RP, 공식 누적 171경기, 33페이지에서 확보한 랭크 171경기, 최근 최대 100경기 기본 리뷰의 도구 4개 실행을 확인했다. 이후 상세 기록 404 수정본은 45개 모의 테스트·타입 검사·전체 lint·공개 빌드·배포 dry-run을 통과하고 커밋 `2ab80ee`로 공개 배포했다. 배포 후 상세 기록 없는 플레이어의 현재·과거 시즌, 물방개 시즌 11의 252경기·39승·7,665 RP, 공개 smoke 회귀 통과와 ER 키·`rules` 설정 유지를 확인했다. 실제 유료 AI·Codex 호출은 하지 않았다. 상세 최신 검사 결과는 [README](../README.md#검증)를 따른다.
