...
2026-09-16에 [공개 사이트](https://lumia-er-reviewer.57872222k.workers.dev/)의 실제 ER 검색·시즌 전적·규칙 기반 리뷰를 검증했다. Worker 이름은 `lumia-er-reviewer`이며 `ER_API_KEY` Secret이 적용되어 있다. 사용자 지정 도메인은 연결하지 않았고 `workers.dev` 주소를 사용한다. 기존 로컬 실행과 Codex 설정은 유지한다. 이후 상세 기록이 없는 플레이어를 위한 수정의 로컬 검증은 아래에 구분하며 최신 공개 반영 결과는 README를 따른다.
...
2026-09-16에는 키가 정상 적용된 상태에서 기존 플레이어의 최근 경기 응답이 404여서 검색이 실패했다. 로컬 수정본은 경기 목록의 404만 빈 상세 기록으로 처리하고 시즌 목록과 API가 제공하는 누적 성적은 계속 조회한다. 이전 시즌을 선택해도 API가 그 UID에 대해 누적 기록을 제공하지 않으면 임의로 복원하지 않는다. 인증·접근 오류, 호출 제한, 다른 서버 오류는 빈 경기로 바꾸지 않는다. 이 수정본의 공개 배포 여부는 [README 검증](../README.md#검증)을 따른다.
...
2026-09-16 공개 검증: 명시적 HTTP 검사 13개와 smoke 검사를 통과했다. 키가 등록되어 미설정 키 오류 항목은 건너뛰었다. 물방개 시즌 12의 7,186 RP, 공식 누적 171경기, 33페이지에서 확보한 랭크 171경기, 최근 최대 100경기 기본 리뷰의 도구 4개 실행을 확인했다. 실제 유료 AI·Codex 호출은 하지 않았다. 이후 상세 기록 404 수정본은 45개 모의 테스트와 타입 검사·공개 빌드·배포 dry-run을 통과했다. 이 수정의 동작 검증은 로컬 기준이다. 상세 최신 검사 결과는 [README](../README.md#검증)를 따른다.
