...
실제 전적 조회와 Codex 예시 리뷰는 각각 검증했습니다. 본인 실제 전적 검색부터 Codex 리뷰까지 이어지는 수행평가용 최종 시연·캡처는 별도로 확보해야 합니다.
...
현재 구현·개발 문서는 준비되어 있습니다. 본인 전적의 최종 시연·캡처, 직접 작성한 실사용 후기, 블로그/SNS 게시 링크, GitHub 저장소·커밋 이력 정리는 제출 전 남은 작업입니다. 2026-09-15 확인 시 결과보고서는 안내문과 인적 사항만 있고 본문·캡처·링크가 비어 있었습니다.

[과제 안내문](https://docs.google.com/document/d/1U5KLOvj3kATbzr4lgBHIQqg6ji5wFsUb/edit)은 코드·실행 화면·개발 문서·후기·블로그/SNS 링크·GitHub 링크를 요구하며 공개 서비스 URL 제출은 명시하지 않습니다. 완성도 항목의 “정상 배포·실행”에 외부 배포가 포함되는지는 교사의 별도 안내를 확인해야 합니다.

## GitHub 저장소

소스 저장소: [jiho08/ER-Search-Reviewer](https://github.com/jiho08/ER-Search-Reviewer)

현재 작업 폴더의 `origin`을 이 저장소에 연결했습니다. 기존 GitHub README의 프로젝트명과 소개를 이 문서에 반영하고, 기존 저장소 이력과 로컬 개발 이력을 함께 보존해 업로드를 준비했습니다. 업로드 완료 여부는 아래 개발 기록과 저장소의 `main`에서 확인합니다.
...
수정 작업마다 기능·화면·실행법·설정·제약·검증 결과 중 달라진 내용을 이 README에 함께 반영합니다. 개발 경위는 `docs/development.md`, 수행평가 준비 상태는 `docs/assignment.md`에 기록하며, 연결된 안내 문서와 현재 상태가 어긋나지 않도록 확인합니다. 실행하지 않은 검사는 통과로 기록하지 않고, 과거 검증은 날짜와 범위를 유지합니다. 이후 작업 지침은 [AGENTS.md](AGENTS.md)에 정리했습니다.
