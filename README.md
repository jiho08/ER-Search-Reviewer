...
- 최근 상세 기록이 없는 기존 플레이어도 시즌 목록과 API가 제공하는 시즌 누적 성적을 확인하도록 수정하고 공개 사이트에 반영했습니다. 빈 상세 기록은 닉네임 오류와 구분하고, 이전 시즌 선택을 안내하며 분석할 경기가 없으면 리뷰 버튼을 비활성화합니다.
...
| 상세 경기 없는 플레이어 수정·공개 배포 | 2026-09-16 | 커밋 `2ab80ee`의 Cloudflare 빌드·배포 성공. 공개 API에서 병관이아트수제자 시즌 12·11 모두 HTTP 200, 상세·누적 0경기·시즌 목록 41개 확인. 물방개 시즌 11 누적 252경기·39승·7,665 RP 확인. 배포 후 공개 smoke 통과(키 미설정 항목은 건너뜀), ER 연결·기본 리뷰 설정 유지. 공개 브라우저에서 검색·이전 시즌 안내·리뷰 비활성화, 시즌 11 선택 시 선택값·랭크 카드·시즌 날짜 변경 확인 |
| lint | 2026-09-16 | 수정본 전체 `npm run lint` 종료 코드 0, 오류·경고 없이 통과 |
