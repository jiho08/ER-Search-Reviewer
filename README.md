...
**[공개 사이트 바로가기](https://lumia-er-reviewer.57872222k.workers.dev/)**에서 설치 없이 전적을 검색하고 기본 리뷰를 받을 수 있습니다. 공개 서버의 실제 ER API 연결과 시즌 전적 조회를 검증했습니다. 개인 PC에서 실행하면 로그인된 Codex 계정으로 AI 리뷰를 사용할 수 있으며, ER API 키 없이도 예시 20경기와 기본 리뷰를 실행할 수 있습니다.
...
- 최근 상세 기록이 없는 기존 플레이어도 시즌 목록과 API가 제공하는 시즌 누적 성적을 확인하도록 수정했습니다. 빈 상세 기록은 닉네임 오류와 구분하고, 이전 시즌 선택을 안내하며 분석할 경기가 없으면 리뷰 버튼을 비활성화합니다. 이 수정의 검증은 아래에 로컬 확인 범위로 구분해 기록합니다.
...
2026-09-16에 [Cloudflare 공개 사이트](https://lumia-er-reviewer.57872222k.workers.dev/)의 화면·전적 검색·시즌 이어 불러오기·기본 리뷰를 확인했습니다. `ER_API_KEY`는 공개 서버에 적용되어 있고 공개 리뷰는 규칙 기반 분석입니다. 계정 연결, GitHub 자동 배포, Secret 등록 절차는 [Cloudflare 배포 안내](docs/cloudflare-deployment.md)에 정리했습니다.
...
React 19, TypeScript, Next.js 호환 App Router(Vinext), Radix UI, Tailwind CSS, Zod, Recharts를 사용합니다. 공개 사이트는 Cloudflare Workers와 Durable Object의 SQLite 저장소를 사용합니다. 사이트 회원 로그인·설치형 프로그램은 현재 버전에 포함하지 않았습니다. Codex 계정 로그인은 로컬 리뷰 연결에 사용합니다.
...
| 검증 범위 | 최근 확인일 | 결과 |
| --- | --- | --- |
| 자동 테스트·타입 검사 | 2026-09-16 | 45개 테스트와 타입 검사 통과. 상세 경기 404의 빈 결과 처리·페이지 종료·인증/호출 제한 오류 유지·시즌 무전적과 미제공 구분 포함. 테스트는 모의 응답 사용 |
| 공개 사이트 HTTP·실제 ER 연결 | 2026-09-16 | 상세 경기 404 수정 전 공개 버전에서 명시적 검사 13개와 smoke 검사 통과. 키가 등록되어 미설정 키 오류 항목은 건너뜀. 물방개 시즌 12: 7,186 RP, 공식 171경기와 33페이지에서 확보한 랭크 171경기 일치. 최근 최대 100경기 기본 리뷰·분석 도구 4개 확인. 유료 AI·Codex 호출 없음 |
| 상세 경기 없는 플레이어 수정 | 2026-09-16 | 로컬에서 병관이아트수제자의 현재 시즌 12·과거 시즌 11 조회가 HTTP 200, 상세 0경기·시즌 목록 41개로 응답. 물방개 과거 시즌 11 누적 252경기·39승·7,665 RP 확인. 브라우저에서 시즌 11 전환과 리뷰 비활성화 확인. 이 항목은 로컬 검증 |
| lint | 2026-09-16 | 상세 경기 404 수정의 변경 코드 5개 파일 대상 ESLint 오류·경고 0개. 전체 lint는 실행 시간이 길어 중단했으므로 이번 수정본 전체 검사 완료로 기록하지 않음. 공개 배포 준비 버전의 전체 통과 기록은 이전 검증 |
| 상세 경기 404 수정본 공개용 빌드·dry-run | 2026-09-16 | 공개용 빌드와 업로드 없는 Wrangler dry-run 통과. 기존 청크 크기·동적 import·플러그인 처리 시간 경고가 남음 |
| 공개용 빌드·배포 dry-run·로컬 HTTP | 2026-09-16 | 공개 배포 준비 버전에서 HTML·예시 20경기·기본 리뷰·입력 검증·헤더 위조 시 호출 제한·연속 오류 응답 검사 통과. 기존 로컬 서버 HTTP 회귀 검사도 통과. 당시 dry-run은 업로드 없이 실행 |
...
`npm run build`는 로컬 실행용 결과를 `dist`에 생성합니다. `npm start`로 빌드된 Worker 서버를 로컬에서 실행할 수 있습니다(기본 http://127.0.0.1:8787). `node scripts/smoke.mjs http://127.0.0.1:8787`로 이 서버도 검증할 수 있습니다. 공개 사이트 갱신은 별도의 공개용 빌드·배포 경로를 사용하며 사용자 지정 도메인은 아직 연결하지 않았습니다. 개발 중 미리보기에는 `npm run dev`를 사용하세요.
...
- 과거 시즌의 공식 누적 성적은 상세 기록과 별도 API에서 조회합니다. 상세 기록이 0경기여도 누적 성적이 제공될 수 있습니다. 누적 API가 명시적으로 빈 배열을 반환한 경우만 0경기·0승으로 표시하며, 응답 실패나 필드 누락을 0으로 간주하지 않습니다.
...
- 로컬 요청 제한은 인스턴스 기준이며 공개용은 Durable Object에서 공유합니다. 공개용 제한과 AI 일일 건수도 재시작 후 유지됩니다. 실제 Cloudflare 배포와 ER 연결은 확인했으나 다수 사용자의 동시 이용 성능은 아직 검증하지 않았습니다. 개인키의 호출량과 무료 Workers CPU 한도 때문에 많은 사용자에게 즉시 응답하는 서비스는 보장하지 않습니다.
