...
// npm's cmd.exe bin shims misparse an ampersand in a checkout path.
// PowerShell's generated shims quote those paths; keep other platforms unchanged.
if (process.platform === "win32") {
  process.env.npm_config_script_shell ??= path.join(
...
if (readExecutionProfile() === "managed-linux") {
  const result = spawnSync(
    "bash",
    [path.join(projectRoot, "scripts/install-ci.sh")],
    {
      stdio: "inherit",
    },
  );
